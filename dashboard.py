...
DATA = r"C:\Users\User\Downloads\community_dashboard\data"

@st.cache_data
def load_data():
    unemp = pd.read_csv(DATA + r"\USA_Unemployment_data.csv")
    ...
    edu = pd.read_csv(DATA + r"\arizona_education.csv")
    crime = pd.read_csv(DATA + r"\arizona_crime.csv")

    counties = gpd.read_file(DATA + r"\tl_2023_us_county.shp")
    ...
    resources = gpd.read_file(DATA + r"\phoenix_community_resources.geojson")
...
selected_counties = st.sidebar.multiselect(
    "Select Counties", all_counties, default=all_counties
)
...
with col2:
    ...
    fig_bar = px.bar(
        ...
        labels={col_name: layer, "NAME": "County"},
        height=480
    )
...
# ── Scatter + Vulnerability bar ───────────────────────────────────────────────
col3, col4 = st.columns(2)

with col3:
    st.subheader("Unemployment vs Education Gap")
    scatter_df = filtered.dropna(subset=["Less_than_HS_pct","Unemployment_Rate","Resource_Count"]).copy()
    scatter_df["Resource_Count_size"] = scatter_df["Resource_Count"].clip(lower=1)
    fig_sc = px.scatter(
        scatter_df,
        x="Less_than_HS_pct",
        y="Unemployment_Rate",
        size="Resource_Count_size",
        color="Vulnerability_Score",
        color_continuous_scale="OrRd",
        hover_name="NAME",
        size_max=40,
        labels={
            "Less_than_HS_pct": "Less than HS %",
            "Unemployment_Rate": "Unemployment Rate %",
            "Resource_Count_size": "Resource Count"
        }
    )
...
display_df = filtered[show_cols].rename(columns={
    ...
    "Less_than_HS_pct":"Less than HS %",
    "Bachelors_or_higher_pct":"Bachelors %",
